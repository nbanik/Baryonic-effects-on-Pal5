...
def set_potential(potname='MWPotential2014'):
    '''
    potname either MWPotential2014
    or index of potential from the param_file
    '''
    ...
    if potname=='MWPotential2014':
        ...
        pind=paramf[potname][0]

        peri=round(paramf[potname][1],2)
        print (peri)


        flat_c=paramf[potname][2]
        vc=paramf[potname][3]
        tvo= vc*_REFV0
...
def aparxv_stream_from_pkl(pot=MWPotential2014,sampling=256,nchunks=16):
    ...
    if nchunks > 1 :
        ...
        for i in range(nchunks):
            ...
            with open('pkl_files/pal5pepper_{}sampling_pot8_peri6.41_{}.pkl'.format(sampling,i),'rb') as savefile:
