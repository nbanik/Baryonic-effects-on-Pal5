...
for ii in range(len(pat_speed)) :
        ...
        folder='/ufrc/tan/nilanjan1/galpy/sampled_SCFbar/1010Msun_{}patspeed_spraydf/trailing/'.format(int(pat_speed[ii]))
        folder_lead=folder.replace('trailing','leading')
        
        ft1=ft.replace('57',str(int(pat_speed[ii])))
        fl1=fl.replace('57',str(int(pat_speed[ii])))
        ...
        barpot,nobarpot=SCFbar_util.Particle_Spray_MWPotentialSCFbar(mbar=Mbar,Acos=Ac,Asin=As,t_on=t_on,pat_speed=pat_speed[ii],tstream=td)
