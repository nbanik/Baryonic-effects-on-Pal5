...
for ii in range(len(Mbar)):
    ...
    folder='/ufrc/tan/nilanjan1/galpy/sampled_SCFbar/' + Mbar_str[ii] + 'Msun_{}patspeed_spraydf/trailing/'.format(int(pat_speed))
    folder_lead=folder.replace('trailing','leading')
    
    ft1=ft.replace('1010',Mbar_str[ii])
    fl1=fl.replace('1010',Mbar_str[ii])
    ...
    barpot,nobarpot=SCFbar_util.Particle_Spray_MWPotentialSCFbar(mbar=Mbar[ii],Acos=Ac,Asin=As,t_on=t_on,pat_speed=pat_speed,tstream=td)
